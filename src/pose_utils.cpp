...
colvec ypr_to_quaternion(const colvec& ypr)
{
colvec q(4);
double y_2 = ypr(0)/2;
double p_2 = ypr(1)/2;
...
q(0) = cos(r_2)*cos(p_2)*cos(y_2) + sin(r_2)*sin(p_2)*sin(y_2);
q(1) = sin(r_2)*cos(p_2)*cos(y_2) - cos(r_2)*sin(p_2)*sin(y_2);
q(2) = cos(r_2)*sin(p_2)*cos(y_2) + sin(r_2)*cos(p_2)*sin(y_2);
q(3) = cos(r_2)*cos(p_2)*sin(y_2) - sin(r_2)*sin(p_2)*cos(y_2);
return q;
}
